...
@pytest.fixture
def override_driver():
    """Auth de motorista fake (testes unitários que mockam o service)."""
...
def override_passanger():
    """Auth de passageiro fake (testes unitários que mockam o service)."""
    app.dependency_overrides[get_current_user] = lambda: FAKE_PASSANGER
    try:
        yield FAKE_PASSANGER
    finally:
...
def test_accept_request_success_returns_200(override_driver) -> None:
    ...
    response = client.post(f"/routes/{uuid.uuid4()}/passangers/{uuid.uuid4()}/accept")
...
def test_accept_request_route_not_found_returns_404(override_driver) -> None:
...
def test_accept_request_wrong_owner_returns_403(override_driver) -> None:
...
def test_accept_request_in_progress_returns_409(override_driver) -> None:
...
def test_accept_request_rp_not_found_returns_404(override_driver) -> None:
...
def test_accept_request_already_processed_returns_409(override_driver) -> None:
...
def test_accept_request_capacity_exceeded_returns_409(override_driver) -> None:
...
def test_reject_request_success_returns_200(override_driver) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.reject_request.return_value = make_rp_response(status="rejected")
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service

    response = client.post(f"/routes/{uuid.uuid4()}/passangers/{uuid.uuid4()}/reject")
...
def test_reject_request_rp_not_found_returns_404(override_driver) -> None:
    from src.domains.route_passangers.errors import RoutePassangerNotFoundError
...
def test_reject_request_route_not_found_returns_404(override_driver) -> None:
    from src.domains.routes.errors import RouteNotFoundError
...
def test_reject_request_wrong_owner_returns_403(override_driver) -> None:
    from src.domains.routes.errors import RouteOwnershipError
...
def test_reject_request_in_progress_returns_409(override_driver) -> None:
    from src.domains.routes.errors import RouteInProgressError
...
def test_reject_request_already_processed_returns_409(override_driver) -> None:
    from src.domains.route_passangers.errors import RoutePassangerAlreadyProcessedError
...
def test_remove_passanger_success_returns_204(override_driver) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.remove_passanger.return_value = None
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service

    response = client.delete(f"/routes/{uuid.uuid4()}/passangers/{uuid.uuid4()}")

    app.dependency_overrides.clear()
    assert response.status_code == 204


def test_remove_passanger_not_found_returns_404(override_driver) -> None:
    from src.domains.route_passangers.errors import RoutePassangerNotFoundError
...
def test_remove_passanger_route_not_found_returns_404(override_driver) -> None:
    from src.domains.routes.errors import RouteNotFoundError
...
def test_remove_passanger_wrong_owner_returns_403(override_driver) -> None:
    from src.domains.routes.errors import RouteOwnershipError
...
def test_remove_passanger_in_progress_returns_409(override_driver) -> None:
    from src.domains.routes.errors import RouteInProgressError
...
def test_list_passangers_success_returns_200(override_driver) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    ...
    response = client.get(f"/routes/{uuid.uuid4()}/passangers?status=pending")
...
def test_list_passangers_filters_by_status(override_driver) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.list_by_status.return_value = []
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service

    route_id = uuid.uuid4()
    client.get(f"/routes/{route_id}/passangers?status=accepted")
...
def test_list_passangers_no_filter_returns_all(override_driver) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.list_by_status.return_value = [
        make_rp_response(status="pending"),
    ...
    response = client.get(f"/routes/{uuid.uuid4()}/passangers")
...
def test_list_passangers_route_not_found_returns_404(override_driver) -> None:
    from src.domains.routes.errors import RouteNotFoundError
...
def test_list_passangers_wrong_owner_returns_403(override_driver) -> None:
    from src.domains.routes.errors import RouteOwnershipError
...
def test_list_passangers_invalid_status_returns_422(override_driver) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.list_by_status.side_effect = ValueError("status inválido")
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service

    response = client.get(f"/routes/{uuid.uuid4()}/passangers?status=foobar")

    app.dependency_overrides.clear()
    assert response.status_code in (400, 422)


def test_list_passangers_response_includes_names(override_driver) -> None:
...
def test_integration_accept_request_success(integration_client, db_session, auth_header) -> None:
...
def test_integration_accept_request_in_progress_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
...
def test_integration_accept_request_capacity_exceeded_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
...
def test_integration_accept_request_wrong_owner_returns_403(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
...
def test_integration_accept_request_persists_stop_in_db(integration_client, db_session, auth_header) -> None:
...
def test_integration_accept_request_stop_type_matches_outbound(integration_client, db_session, auth_header) -> None:
...
def test_integration_reject_request_success(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
    rp = make_integration_rp(db_session, route.id, passenger.id)
...
def test_integration_reject_request_in_progress_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id, status="em_andamento")
    rp = make_integration_rp(db_session, route.id, passenger.id)
...
def test_integration_reject_request_already_processed_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
...
def test_integration_reject_request_wrong_owner_returns_403(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    other_driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
...
def test_integration_remove_passanger_success(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
    rp = make_integration_rp(db_session, route.id, passenger.id, status="accepted")
...
def test_integration_remove_passanger_in_progress_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id, status="em_andamento")
    rp = make_integration_rp(db_session, route.id, passenger.id, status="accepted")
...
def test_integration_remove_passanger_wrong_owner_returns_403(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    other_driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
...
def test_integration_remove_passanger_deletes_stop_from_db(integration_client, db_session, auth_header) -> None:
...
def test_integration_list_passangers_filter_pending(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    route = make_integration_route(db_session, driver.id)
...
def test_integration_list_passangers_no_filter_returns_all(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    route = make_integration_route(db_session, driver.id)
    p1 = make_integration_passenger(db_session, "P1")
    p2 = make_integration_passenger(db_session, "P2")
...
def test_integration_list_passangers_resolves_user_name(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    route = make_integration_route(db_session, driver.id)
...
def test_integration_list_passangers_wrong_owner_returns_403(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    other_driver, _ = make_integration_driver(db_session)
    route = make_integration_route(db_session, driver.id)
    headers = auth_header(other_driver)
...
def test_integration_list_passangers_route_not_found_returns_404(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    headers = auth_header(driver)
...
def test_join_route_success_returns_201(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
...
def test_join_route_route_not_found_returns_404(override_passanger) -> None:
    from src.domains.routes.errors import RouteNotFoundError
...
def test_join_route_in_progress_returns_409(override_passanger) -> None:
    from src.domains.routes.errors import RouteInProgressError
...
def test_join_route_full_returns_409(override_passanger) -> None:
    from src.domains.route_passangers.errors import RouteCapacityExceededError
...
def test_join_route_duplicate_returns_409(override_passanger) -> None:
...
def test_join_route_invalid_payload_returns_422(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service
...
def test_integration_join_route_success(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
...
def test_integration_join_route_creates_schedules(integration_client, db_session, auth_header) -> None:
...
def test_integration_join_route_duplicate_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
    make_integration_rp(db_session, route.id, passenger.id, status="pending")
...
def test_integration_join_route_in_progress_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id, status="em_andamento")
    headers = auth_header(passenger)
...
def test_leave_route_success_returns_204(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
...
def test_leave_route_with_dependent_id_query_forwarded(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.leave_route.return_value = None
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service
    dep_id = uuid.uuid4()
...
def test_leave_route_route_not_found_returns_404(override_passanger) -> None:
    from src.domains.routes.errors import RouteNotFoundError
...
def test_leave_route_in_progress_returns_409(override_passanger) -> None:
    from src.domains.routes.errors import RouteInProgressError
...
def test_leave_route_rp_not_found_returns_404(override_passanger) -> None:
    from src.domains.route_passangers.errors import RoutePassangerNotFoundError
...
def test_integration_leave_route_success(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
    make_integration_rp(db_session, route.id, passenger.id, status="pending")
...
def test_integration_leave_route_no_active_rp_returns_404(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
    headers = auth_header(passenger)
...
def test_integration_leave_route_in_progress_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id, status="em_andamento")
...
def test_update_schedules_success_returns_200(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
...
def test_update_schedules_with_dependent_id_query_forwarded(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
...
def test_update_schedules_route_not_found_returns_404(override_passanger) -> None:
    from src.domains.routes.errors import RouteNotFoundError
...
def test_update_schedules_in_progress_returns_409(override_passanger) -> None:
    from src.domains.routes.errors import RouteInProgressError
...
def test_update_schedules_rp_not_found_returns_404(override_passanger) -> None:
    from src.domains.route_passangers.errors import RoutePassangerNotFoundError
...
def test_update_schedules_invalid_payload_returns_422(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service
...
def test_integration_update_schedules_success(integration_client, db_session, auth_header) -> None:
    from src.domains.route_passangers.schedule_entity import RoutePassangerScheduleModel
...
def test_integration_update_schedules_no_active_rp_returns_404(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id)
    address = make_integration_pickup_address(db_session, passenger.id)
...
def test_integration_update_schedules_in_progress_returns_409(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route = make_integration_route(db_session, driver.id, status="em_andamento")
    address = make_integration_pickup_address(db_session, passenger.id)
...
def test_list_my_routes_success_returns_200(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
...
def test_list_my_routes_empty_returns_200_with_empty_list(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.list_my_routes.return_value = []
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service
...
def test_list_my_routes_calls_service_with_user_id() -> None:
    ...
    user_id = uuid.uuid4()
    caller = UserModel(id=user_id, name="Caller", email="c@test.com", role="guardian")
    app.dependency_overrides[get_current_user] = lambda: caller
    client.get("/routes/me")

    app.dependency_overrides.clear()
    assert mock_service.list_my_routes.call_args.args[0] == user_id


def test_list_my_routes_exposes_dependent_name_when_present(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.list_my_routes.return_value = [
...
def test_list_my_routes_exposes_driver_phone(override_passanger) -> None:
...
def test_integration_list_my_routes_returns_active_memberships(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
...
def test_integration_list_my_routes_empty_returns_empty_list(integration_client, db_session, auth_header) -> None:
    passenger = make_integration_passenger(db_session)
    headers = auth_header(passenger)
...
def test_integration_list_my_routes_ignores_rejected_and_removed(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
    passenger = make_integration_passenger(db_session)
    route_a = make_integration_route(db_session, driver.id)
    route_b = make_integration_route(db_session, driver.id)
...
def test_integration_list_my_routes_does_not_collide_with_route_by_id(integration_client, db_session, auth_header) -> None:
...
def test_get_my_route_detail_success_returns_200(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.get_my_route_detail.return_value = make_detail_response()
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service

    response = client.get(f"/routes/{uuid.uuid4()}/me")
...
def test_get_my_route_detail_does_not_expose_driver_id_or_invite_code(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    mock_service.get_my_route_detail.return_value = make_detail_response()
    app.dependency_overrides[get_route_passanger_service] = lambda: mock_service
...
def test_get_my_route_detail_route_not_found_returns_404(override_passanger) -> None:
    from src.domains.routes.errors import RouteNotFoundError
...
def test_get_my_route_detail_not_passanger_returns_403(override_passanger) -> None:
...
def test_get_my_route_detail_forwards_dependent_id_query_param(override_passanger) -> None:
    mock_service = Mock(spec=RoutePassangerService)
    ...
    route_id = uuid.uuid4()
    dep_id = uuid.uuid4()
    response = client.get(f"/routes/{route_id}/me?dependent_id={dep_id}")
...
def test_get_my_route_detail_current_trip_id_when_in_progress(override_passanger) -> None:
    trip_id = uuid.uuid4()
    mock_service = Mock(spec=RoutePassangerService)
...
def test_integration_get_my_route_detail_success(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
...
def test_integration_get_my_route_detail_route_not_found_returns_404(integration_client, db_session, auth_header) -> None:
    passenger = make_integration_passenger(db_session)
    headers = auth_header(passenger)
...
def test_integration_get_my_route_detail_not_passanger_returns_403(integration_client, db_session, auth_header) -> None:
    driver, _ = make_integration_driver(db_session)
...
def test_integration_get_my_route_detail_pending_membership_allowed(integration_client, db_session, auth_header) -> None:
...
def test_integration_get_my_route_detail_does_not_collide_with_route_by_id(integration_client, db_session, auth_header) -> None:
...
def test_integration_get_my_route_detail_rejected_membership_returns_403(integration_client, db_session, auth_header) -> None:
